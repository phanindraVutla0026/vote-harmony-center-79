...
const CandidateCard: React.FC<CandidateCardProps> = ({
  ...
  const districtName = getDistrictNameById(
    candidate.stateId,
    candidate.districtId
  );
  const mandalName = getMandalNameById(
    candidate.stateId,
    candidate.districtId,
    candidate.mandalId
  );
          ...
          {votingEnabled && (
            <button
              ...
              className={`w-full py-2 px-4 rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                isSelected
                  ? "bg-red-500 text-white cursor-default" // Red when voted
                  : "bg-green-500 text-white hover:bg-green-600 focus:ring-green-500" // Green when not voted
              }`}
            >
